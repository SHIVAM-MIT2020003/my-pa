...
import base64
import time
from mypa.audio_recorder.tasks import call_twilio
...
def homepage(request,id):
    # call_twilio.delay()
    return render(request, '../templates/home.html', {"id": id})



@csrf_exempt
def store_mp3(request):
    if request.method == 'POST':
        u = dict(request.POST)
        for k, v in u.items():
            s = ''.join(v)
            b = base64.b64decode(s[22:])
            try:
                moment = time.strftime("%Y-%b-%d__%H_%M_%S", time.localtime())
                with open('media/meeting' + moment + '.mp3', 'wb') as destination:
                    destination.write(b)
                # text_file = open('media/meeting' + moment + '.txt', 'w')
                # text_file.write("%s" % s[22:])
